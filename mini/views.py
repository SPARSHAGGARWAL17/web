...
def check(request,money=0):
    if request.method == "POST":
        ...
        for i in cardDetails:
            ...
            if i.name == name and str(i.date) == date and i.cvv == int(cvv) and i.card == int(card) and i.email == str(request.user):
                print('verified')
                for reg in register:
                    reg.money = money
                    reg.save()
                ...
                print("fraud = ",fraudNo)
                if fraudNo == 3:
                    return render(request,'mini/fraud.html',context={'STATIC_URLS':STATIC_URL,'number':number})
...
def verify(request):
    contexts = {'STATIC_URLS': STATIC_URL, 'button': 'VERIFY'}
    if request.method=="POST" and contexts['button']=='VERIFY':
        print('if')
    ...
    for i in register:
        ...
        if request.method=="POST":
            if request.POST.get('card')!='':
                ...
                return render(request,'mini/addcard.html', {'STATIC_URLS': STATIC_URL, 'button': 'VERIFY','money':money})
